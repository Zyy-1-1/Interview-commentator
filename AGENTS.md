...
前端用 `npm.cmd test` 运行 Vue 组件离线回归，覆盖条件渲染、失败重试和图表生命周期；内存 DOM 检查不能代替浏览器端到端验收。目前没有 lint 或浏览器端到端测试脚本。当前 Compose 包含后端和 Nginx 前端；配置校验、镜像构建、容器运行和整站浏览器验收是不同结果，分别记录。默认端口仅绑定本机；验收使用独立项目、空密钥和临时数据，避免影响已有数据卷。
...
- 面试结束与报告生成成功分别处理；报告在后台线程生成，通过数据库领取避免重复调用。保留任务编号校验、过期重试和只读状态查询；评估失败可见，不能提前宣称报告已生成。
