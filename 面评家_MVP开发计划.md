...
- **核心理由**:开源项目多为「题库刷题 + 一次性问答」的陪练工具,而面评家是「带状态机的 AI 模拟面试官」——按 JD 拆成能力维度逐维度动态追问,结束后输出带原话证据的**个人竞争力报告**(强项短板 + 提升建议),帮应聘者练题、发现差距;该闭环(状态机驱动 + 证据化个人评估)无现成开源;且 offerMaster/AURORA 无 LICENSE,DeepInterview 是巨型语音优先仓库。
- **目标**:6 周内做出可现场演示的完整闭环,答辩叙事 =「自研 LangGraph 状态机 + 动态追问 + 证据引用 + 标准化个人评估报告」。
...
**MVP 必须完整闭环**:应聘者选择目标岗位/粘贴 JD → 上传简历(可选)→ 生成考察大纲 → 发起模拟面试 → 免登录答题 → AI 动态追问 → 收尾自动评估 → 应聘者查看个人竞争力报告。
...
| 前端 | Vue3 + Vite + Element Plus(应聘者面试页 + 演示后台) |
| 图表 | echarts(雷达图) |
| 部署 | docker compose |
...
### W4 — 评估 Agent + 演示后台
- [x] 评估 Agent:1 次 LLM 调用出个人竞争力报告 JSON(方案 4.5)+ 证据引用原话
- [x] 收尾自动触发评估 + `POST /evaluate` 手动触发;`GET /report` 已可用
- [ ] 报告页:雷达图(echarts)+ 得分 + 提升建议(前端)
- [ ] 演示后台:上传简历 → 发起模拟面试 → 查看报告(前端)
- ✅ **验收(后端)**:收尾自动出报告(集成测试断言通过);面试结束报告落库

### W5 — 打磨 + 演示数据 + 横向对比
- [x] `make_demo.py`(5 岗位 + 20 简历 + 预置面试,2026-08-31)
- [x] 多应聘者横向对比表(演示用,精简版):`GET /api/interviews/comparison` 按岗位分组,前端 `/admin/comparison`
- ✅ **验收**:完整闭环可演示

### W6 — 部署 + README + 答辩材料
- [x] docker compose 编排 + 后端 Dockerfile(2026-08-30 已建,待端到端验证)
- [x] README:架构 + 部署 + **三组验收指标**(2026-08-31)
- [x] 演示脚本(方案 11.2,2026-08-31)+ 答辩材料大纲已并入脚本附节
- ✅ **验收**:现场可跑完整演示
...
- [x] 前端全部完成(应聘者面试页 `dc9dc74` + 演示后台/报告页 `776ac61`)
- [x] W5:演示数据(make_demo.py)
- [x] W5:多候选人横向对比表(精简版)
- [x] W5:打磨(演示数据 + 对比页就绪)
- [x] W6:README(架构 + 部署 + 三组验收指标)
- [x] W6:演示脚本(scripts/演示脚本.md)
- [ ] W6:docker 端到端验证 + 答辩 PPT
