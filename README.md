...
报告状态为 `not_started / pending / running / ready / failed`，`can_retry` 表示当前是否可发起评估。收尾请求落库后即安排后台线程评估，页面每 2 秒查询状态。数据库领取与任务编号阻止同一报告并发重复调用及旧任务覆盖新结果。当前使用进程内后台任务：服务重启不会自动恢复未完成任务，超过 210 秒（或报告模型总预算加 30 秒，取较大值）后可在报告页手动重试；查询状态不会触发模型。多节点部署或大规模队列需要独立任务系统。

模型默认单次网络超时 `LLM_TIMEOUT_SECONDS=60`，多次尝试共享 `LLM_TOTAL_TIMEOUT_SECONDS=120` 秒重试预算（上限 120）；每次请求使用剩余预算收紧超时，鉴权与参数错误不重复请求。竞争力报告使用独立的 `REPORT_LLM_TIMEOUT_SECONDS=90` 与 `REPORT_LLM_TOTAL_TIMEOUT_SECONDS=180` 秒预算，最多重试一次。网络库的超时不等同于严格的进程执行时限。生成报告已从答题请求中分离。
